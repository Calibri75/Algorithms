...
void generation(const string& filename){
    ...
    string alpha = "aeiouybcdfghjklmnpqrstvwxz !?.,";
    ...
    uniform_int_distribution<> dist_desc(0, static_cast<int>(alpha.size() - 1));

    for(int i = 0; i < 1000000; i++){
        string name, surname, patronymic, description;
        for(int j = 0; j < dist_long(gen); j++){
            description += alpha[dist_desc(gen)];
            if(j % 2 == 0) {
        ...
        file << dist_day(gen) << " " << dist_month(gen) << " " << dist_year(gen) << " "
             << name << " " << surname << " " << patronymic << " " << dist_application(gen) << " " << description << "\n";
